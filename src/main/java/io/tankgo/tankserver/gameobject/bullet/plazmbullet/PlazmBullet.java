...
@Slf4j
public class PlazmBullet extends BaseBullet {

    @Override
    public void init(BulletParam param, World world, double worldTime) {
        this.world = world;
        this.setWorldTime(worldTime);
        this.setGameObjectState(new PlazmBulletIdel(this));
        bullet = new Body();
        bullet.addFixture(Geometry.createCircle(0.2),
                500,
                0.1,
                0.1);
...
//        bullet.setAngularDamping(Integer.MAX_VALUE);
        ...
        double vx = Math.cos(fireVec.getR())*0.7;
        double vy = Math.sin(fireVec.getR())*0.7;
